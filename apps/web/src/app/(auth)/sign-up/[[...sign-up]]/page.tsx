  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      ...
      if (result.status === 'complete') {
        ...
        setError('Verificación incompleta. Inténtalo de nuevo.')
      }
    } catch (err) {
      setError(mapClerkError(err))
    } finally {
    ...
    return (
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="text-center text-sm space-y-2">
          {resendSent && (
            <p className="text-green-600">¡Nuevo código enviado!</p>
          )}
          <button
            type="button"
          ...
          >
            {resendLoading ? 'Enviando...' : '¿No lo recibiste? Reenviar código de verificación'}
          </button>
        </div>
