...
import type { getEmployee } from '@/app/actions/employees'

type EmployeeData = NonNullable<Awaited<ReturnType<typeof getEmployee>>>
...
export function EmployeeEditForm({ workspaceId, employee }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await updateEmployee(employee.id, workspaceId, {
        ...
        router.push(`/workspace/${workspaceId}/employees/${employee.id}`)
      } catch (err) {
    ...
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">{employee.firstName} {employee.lastName}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="border rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Salario bruto anual (€) *">
              <input required type="number" min={0} step={100} value={form.annualGrossSalary} onChange={e => set('annualGrossSalary', e.target.value)} />
            </Field>
        ...
        <section className="border rounded-xl p-5 space-y-4">
          ...
          {form.irpfManual && (
            <Field label="% IRPF">
              <input type="number" min={0} max={45} step={0.5} value={form.irpfPct ?? ''} onChange={e => set('irpfPct', e.target.value ? Number(e.target.value) : null)} />
            </Field>
          )}
        </section>
