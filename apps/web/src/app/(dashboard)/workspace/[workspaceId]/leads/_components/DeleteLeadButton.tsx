...
export function DeleteLeadButton({ leadId, workspaceId, leadName, locale }: Props) {
      ...
      {confirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-background border rounded-xl p-6 max-w-sm w-full space-y-4 shadow-xl">
            <h3 className="font-semibold">{t.leadsDeleteModalTitle}</h3>
            <p className="text-sm text-muted-foreground">
              {t.leadsDeleteModalBody} <strong>{leadName}</strong>
            </p>
            <div className="flex gap-3 justify-end">
              ...
              >
                {isPending ? t.leadsDeleting : t.leadsDelete}
              </button>
            </div>
          </div>
        </div>
