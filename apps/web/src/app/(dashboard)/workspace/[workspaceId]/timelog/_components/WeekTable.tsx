...
export function WeekTable({ workspaceId, initialEntries, weekStart, locale }: Props) {
  ...
  const days = Array.from({ length: 7 }, (_, i) => {
    ...
    return { date, dateStr, entry, dayName: dayNames[i] }
  })
    ...
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">
          {monday.toLocaleDateString(locale, { day: 'numeric', month: 'long' })} — {sunday.toLocaleDateString(locale, { day: 'numeric', month: 'long' })}
        </h2>
      ...
      <div className="rounded-xl border bg-card divide-y overflow-hidden">
                ...
                <div className="flex-1 min-w-0">
                  {entry ? (
                    <div className="flex items-center gap-3 text-sm flex-wrap">
                      <span>{formatTime(entry.clockIn, locale)}</span>
                      <span className="text-muted-foreground">→</span>
                      <span>{entry.clockOut
                        ? formatTime(entry.clockOut, locale)
                        : <span className="text-green-600 dark:text-green-400">{t.timelogInProgress}</span>
                      }</span>
                      <span className="text-muted-foreground">{formatDuration(entry)}</span>
                      {entry.imputations.length > 0 && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-primary/10 text-primary">
                          {entry.imputations.length} {t.timelogImputations}
                        </span>
                      )}
                    </div>
                  ) : (
                    <span className="text-sm text-muted-foreground">{isWeekend ? '—' : t.timelogNoClockIn}</span>
                  )}
                </div>
                {entry && (
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      aria-label={t.timelogViewImputations}
                    >
                      {isExpanded ? t.timelogClose : t.timelogImputations}
                    </button>
                    <button
                      onClick={() => setEditingEntry(entry)}
                      disabled={isPending}
                      className="text-xs text-muted-foreground hover:text-foreground disabled:opacity-50"
                      aria-label={t.timelogEditEntryAria.replace('{day}', dayName ?? '')}
                    >
                      ✏️
                    </button>
                    <button
                      ...
                      aria-label={t.timelogDeleteEntryAria.replace('{day}', dayName ?? '')}
                    >
                      🗑️
                    </button>
                  </div>
        ...
        <div className="flex items-center gap-4 px-4 py-3 bg-muted/20 font-semibold">
          <div className="w-10 shrink-0">
            <p className="text-xs">{t.timelogTotal}</p>
          </div>
