    ...
    <div className="space-y-8">
      ...
      <section className="rounded-xl border bg-card p-6 space-y-4">
        ...
        <label className="flex items-center gap-2 rounded-lg border border-border px-3 py-2 text-sm">
          ...
          />
          <span>{t.wsLogoShowName}</span>
        </label>

        {logoShowName && (
          <div className="rounded-lg border border-border p-3 space-y-3">
            <p className="text-xs font-medium text-muted-foreground">{t.wsLogoNameAdjust}</p>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <label className="text-xs font-medium text-muted-foreground">
                {t.wsLogoHorizontal}
                <input type="range" min="0" max="300" step="1" value={logoText.x} onChange={(e) => setLogoText((prev) => ({ ...prev, x: Number(e.target.value) }))} className="mt-2 w-full" />
              </label>
              <label className="text-xs font-medium text-muted-foreground">
                {t.wsLogoVertical}
                <input type="range" min="2" max="48" step="1" value={logoText.y} onChange={(e) => setLogoText((prev) => ({ ...prev, y: Number(e.target.value) }))} className="mt-2 w-full" />
              </label>
              <label className="text-xs font-medium text-muted-foreground">
                {t.wsLogoSize}
                <input type="range" min="10" max="34" step="1" value={logoText.size} onChange={(e) => setLogoText((prev) => ({ ...prev, size: Number(e.target.value) }))} className="mt-2 w-full" />
              </label>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="text-xs font-medium text-muted-foreground">
                {t.wsLogoFontColor}
                <input type="color" value={logoText.color} onChange={(e) => setLogoText((prev) => ({ ...prev, color: e.target.value }))} className="mt-2 h-9 w-full rounded-md border border-border bg-background" />
              </label>
              <label className="text-xs font-medium text-muted-foreground">
                {t.wsLogoFont}
                <select value={logoText.font} onChange={(e) => setLogoText((prev) => ({ ...prev, font: e.target.value }))} className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
        ...
        <div className="rounded-lg border border-border bg-background p-4 space-y-3">
          ...
          {emailSettings.mode !== 'mitikus' && (
            <div className="space-y-3 pt-1 border-t border-border">
              ...
              <p className="text-xs font-semibold text-foreground pt-2">{t.wsSmtpTitle}</p>
              <div className="grid gap-3 sm:grid-cols-3">
              ...
              <p className="text-xs font-semibold text-foreground pt-2 border-t border-border">{t.wsImapTitle}</p>
              <div className="grid gap-3 sm:grid-cols-3">
                <label className="sm:col-span-2 space-y-1 text-xs font-medium text-muted-foreground">
                  {t.wsImapServer}
          ...
          {emailTestOk && (
            <p className="rounded-md border border-green-500/30 bg-green-500/10 px-3 py-2 text-xs text-green-600 dark:text-green-300">
              {t.wsEmailConnectionOk}
            </p>
          )}

          <div className="flex flex-wrap justify-end gap-2">
            {emailSettings.mode !== 'mitikus' && (
              <button type="button" onClick={handleEmailTest} disabled={emailTesting || emailSaving} className="px-4 py-2 rounded-md border border-border text-sm font-medium hover:bg-muted transition-colors disabled:opacity-60">
                {emailTesting ? t.wsEmailTesting : emailTestOk ? t.wsEmailTestOk : t.wsEmailTest}
              </button>
            )}
            <button type="button" onClick={handleEmailSave} disabled={emailSaving} className="px-4 py-2 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors disabled:opacity-60">
      ...
      <section className="rounded-xl border bg-card p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {PRESET_COLORS.map((c) => (
            <button key={c} type="button" onClick={() => setBrandColor(c)} style={{ backgroundColor: c }} className={`w-8 h-8 rounded-full transition-transform hover:scale-110 ${brandColor === c ? 'ring-2 ring-offset-2 ring-foreground scale-110' : ''}`} aria-label={`Color ${c}`} />
          ))}
          <input type="color" value={brandColor} onChange={(e) => setBrandColor(e.target.value)} className="w-8 h-8 rounded-full border border-border cursor-pointer" title={t.wsBrandColorCustom} />
        </div>
        <div className="flex items-center gap-3 mt-2">
          ...
          <span className="text-xs text-muted-foreground">{t.wsBrandColorPreview}</span>
        </div>
      </section>
        ...
        <label className={`flex items-start gap-3 rounded-lg border border-border px-4 py-3 ${isOwner ? 'cursor-pointer hover:bg-muted/50 transition-colors' : 'opacity-60 cursor-not-allowed'}`}>
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {restrictCreation ? t.wsPermOnlyAdminsActive : t.wsPermOnlyAdminsInactive}
            </p>
            {!isOwner && (
              <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">{t.wsPermOwnerOnly}</p>
            )}
      ...
      <div className="flex justify-end">
        <button type="button" onClick={handleSave} disabled={saving} className="px-5 py-2 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors disabled:opacity-60">
          {saving ? t.wsSaving : saved ? t.wsSaved : t.wsSave}
        </button>
      </div>
    </div>
  )
