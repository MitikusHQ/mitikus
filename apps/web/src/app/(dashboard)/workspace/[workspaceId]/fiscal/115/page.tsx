...
import { Modelo115Client } from './_components/Modelo115Client'
...
export default async function Modelo115Page({ params }: Props) {
  ...
  const saved = await getFiscalDeclaration(workspaceId, '115', periodo, year)
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        <div>
          <h1 className="text-xl font-semibold">Modelo 115 — Retenciones Arrendamientos</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Retenciones sobre alquileres de inmuebles urbanos · {label}</p>
        </div>
        <Modelo115Client
          periodo={label}
          save={{
