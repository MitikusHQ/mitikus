...
import { Modelo130Client } from './_components/Modelo130Client'
...
export default async function Modelo130Page({ params }: Props) {
  ...
  const saved = await getFiscalDeclaration(workspaceId, '130', periodo, year)
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        <div>
          <h1 className="text-xl font-semibold">Modelo 130 — IRPF Trimestral</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Pago fraccionado · Estimación directa · {label}</p>
        </div>
        <Modelo130Client
          periodo={label}
          save={{
