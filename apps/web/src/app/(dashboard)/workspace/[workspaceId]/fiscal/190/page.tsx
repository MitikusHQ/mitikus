...
import { Modelo190Client } from './_components/Modelo190Client'

interface Props { params: Promise<{ workspaceId: string }> }

export default async function Modelo190Page({ params }: Props) {
  const [{ workspaceId }, user] = await Promise.all([params, requireUser()])
  const workspace = await db.workspace.findFirst({ where: { id: workspaceId, orgId: user.orgId }, select: { id: true } })
  if (!workspace) notFound()

  const year = new Date().getFullYear() - 1
  const saved = await getFiscalDeclaration(workspaceId, '190', 'Anual', year)
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded font-mono font-semibold">190</span>
            <h1 className="text-xl font-semibold">Resumen anual retenciones e ingresos a cuenta</h1>
          </div>
          <p className="text-sm text-muted-foreground mt-1">Ejercicio {year} — Resumen de retenciones practicadas (Mod. 111 + 115)</p>
        </div>
        <Modelo190Client
          year={year}
