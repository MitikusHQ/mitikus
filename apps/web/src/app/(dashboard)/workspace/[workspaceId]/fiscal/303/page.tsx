...
import { Modelo303Client } from './_components/Modelo303Client'
...
export default async function Modelo303Page({ params }: Props) {
  ...
  const saved = await getFiscalDeclaration(workspaceId, '303', periodo, year)
  ...
  return (
    <>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 space-y-6">
        <div>
          <h1 className="text-xl font-semibold">Modelo 303 — IVA Trimestral</h1>
          <p className="text-sm text-muted-foreground mt-0.5">Calculadora orientativa · {label}</p>
        </div>
        <Modelo303Client
          periodo={label}
          save={{
